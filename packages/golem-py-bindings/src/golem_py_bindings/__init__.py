...
def register_bindings(world_name: str) -> None:
    ...
    def try_load(module: str) -> None:
        try:
          sys.modules[f"golem_py_bindings.bindings.{module}"] = importlib.import_module(f".{module}", world_name)
        except ImportError:
          logger.warning(f"Binding module {module} is not available. Functionality relying on it will not work.")
          pass
